...
## Phase 9: the off-policy study

Phase 8 convicted the on-policy gradient — PPO dismantled a working
switching policy it was handed. Phase 9 completes the
{PPO, DQN} × {scratch, BC-init} 2×2 with Double DQN, which estimates
per-action values by averaging TD errors over a replay buffer instead
of the last on-policy batch — exactly the property that should resist
the ~25% per-episode return noise that collapses PPO. Two arms per
correlated cell (`agent/dqn.py`, `run_benchmark.py --study offpolicy`):
**dqn-scratch** (fresh Q-network, epsilon 1.0 → 0.05 over the first
40k of the 80k-step budget) and **dqn-bc** (Q-network initialised from
the committed Phase 8 BC checkpoint; epsilon stays low, 0.05 → 0.01
over 20k steps, so random actions do not wash the handed policy out of
the replay buffer). The BC → Q mapping copies the BC actor verbatim —
identical trunk, so argmax-Q *is* the BC policy at initialisation —
then shifts every Q-value by a constant (argmax-invariant) offset onto
the return scale, mean BC step reward / (1 − γ) from the committed
Phase 8 eval rows (−309 UDP, −925 TCP), so TD refines the value
surface instead of rebuilding it through the wrong scale. Same cells,
seeds, and settings as Phases 7c/8; the ppo / bc / bc-ppo / statics /
greedy-per rows below are the committed measurements, not re-runs.

| config | policy | reward | PDR | goodput [Mbps] | switches/ep |
|---|---|---|---|---|---|
| τ 500/100 ms + UDP | PPO scratch (7c, ≡ route 3) | −618.0 ± 165.7 | 0.770 ± 0.068 | — | 1.0 |
| τ 500/100 ms + UDP | bc-ppo (8, ≡ route 0) | −572.9 ± 140.1 | 0.776 ± 0.057 | — | 0.0 |
| τ 500/100 ms + UDP | bc (clone of teacher) | −618.0 ± 117.4 | 0.913 ± 0.029 | — | 73.3 |
| τ 500/100 ms + UDP | **dqn-scratch** (≡ route 0) | −572.9 ± 140.1 | 0.776 ± 0.057 | — | 0.0 |
| τ 500/100 ms + UDP | **dqn-bc** | −582.2 ± 115.3 | 0.811 ± 0.041 | — | 18.4 |
| τ 500/100 ms + UDP | best static (route 0) | −572.9 ± 140.1 | 0.776 ± 0.057 | — | 0 |
| τ 500/100 ms + UDP | greedy-PER (teacher) | **−484.5 ± 113.8** | **0.914 ± 0.026** | — | 46.0 |
| τ 500/100 ms + TCP | PPO scratch (7c, ≡ route 2) | −2120.2 ± 404.0 | 0.730 ± 0.302 | 0.274 | 1.0 |
| τ 500/100 ms + TCP | bc-ppo (8) | −1925.5 ± 444.1 | 0.869 ± 0.163 | 0.462 | 8.2 |
| τ 500/100 ms + TCP | bc (clone of teacher) | −1850.4 ± 548.8 | 0.918 ± 0.112 | **0.799** | 73.5 |
| τ 500/100 ms + TCP | **dqn-scratch** (≡ route 0) | −1897.6 ± 453.2 | 0.870 ± 0.163 | 0.451 | 0.0 |
| τ 500/100 ms + TCP | **dqn-bc** (≡ route 0) | −1897.6 ± 453.2 | 0.870 ± 0.163 | 0.451 | 0.0 |
| τ 500/100 ms + TCP | best static (route 0) | −1897.6 ± 453.2 | 0.870 ± 0.163 | 0.451 | 0 |
| τ 500/100 ms + TCP | greedy-PER (teacher) | **−1726.8 ± 569.6** | **0.928 ± 0.083** | 0.788 | 46.0 |

Paired per-episode analysis (shared seeds;
`parse_traces.py --study offpolicy --paired`):

| comparison | config | reward Δ | PDR Δ | W/T/L | notes |
|---|---|---|---|---|---|
| dqn-scratch − best-static | UDP | 0.0 ± 0.0 | +0.000 | 0/10/0 | byte-identical to route 0 |
| dqn-scratch − best-static | TCP | 0.0 ± 0.0 | +0.000 | 0/10/0 | byte-identical to route 0 |
| dqn-bc − best-static | UDP | −9.3 ± 48.7 | +0.035 | 3/0/7 | statistical tie, keeps switching |
| dqn-bc − best-static | TCP | 0.0 ± 0.0 | +0.000 | 0/10/0 | byte-identical to route 0 |
| dqn-bc − bc | UDP | +35.8 ± 114.5 | −0.102 | 6/0/4 | flap tax pruned, dodging kept |
| dqn-bc − bc | TCP | −47.2 ± 298.0 | −0.047 | 4/0/6 | init destroyed |

**The completed 2×2, quadrant by quadrant.** *PPO scratch* (7c):
collapses to a constant route chosen by training-seed lottery (routes
3/1/2 across runs) — finds neither switching nor reliable route
selection. *PPO BC-init* (8): destroys the handed switching policy in
both cells. *DQN scratch* (9): also never finds switching — but it
collapses onto the **correct** constant route in both cells,
byte-identical to true best-static route 0 on all 10 seeds (0/10/0).
Averaging value estimates over the whole replay buffer fixes the route
lottery that on-policy batches lose: across-episode averaging ranks
the four routes correctly where PPO locked in early noise. It cannot
manufacture switching, though — under epsilon-greedy dithering a
coherent switch is never *sampled* (random flaps pay the penalty
without dodging fades), so no switching signal ever enters the buffer.
*DQN BC-init* (9): a split verdict, and the most informative cell.
In the UDP cell the switching policy **survives all 80k steps** — the
one quadrant-cell in the whole 2×2 where an RL fine-tune retains
switching. In the TCP cell it is destroyed faster than PPO managed.

The trajectory (`plots/offpolicy_trajectory.png`; per-run CSVs
`offpolicy_trajectory_<arm>_<cell>.csv` — as in Phase 8, the
trajectory is the result) shows both mechanisms. UDP dqn-bc starts at
~82 greedy switches per episode (the hysteresis-free clone), prunes
noisily to ~15–20 by update 40 (20k steps), and then **holds that
equilibrium for the remaining 60k steps** — mean 15.4 (range 8–22)
over the last 40 updates, no monotonic drain — while its Q-gap
(best-minus-second action value, the margin the greedy policy acts on)
*grows* from 1.8 to ~8. Where PPO's entropy drain has one attractor,
TD learning settled on a genuinely state-dependent value surface:
evaluation shows 18.4 switches/ep, +3.5 pp PDR and 85 fewer PHY
drops/ep than best-static, reward a statistical tie with best-static
(−9.3 ± 48.7, 3/7) and better than its own BC initialisation (+35.8,
6/4) — it pruned the flap tax the clone paid and kept the
highest-margin fade dodges, though it prunes past the teacher's 46
switches/ep and gives up much of the PDR with them (0.914 → 0.811 vs
the teacher; reward −484.5 vs −582.2): a diminished but *stable* heir,
not a policy in mid-collapse.

The TCP cell shows the failure mode that survives the switch to
off-policy learning. There the BC-initialised Q-gap (~1.8, the BC
logit margin) met TD errors from step rewards ~3× larger and noisier
than UDP's (per-episode std 548 ≈ ±2.7 per step-value, bigger than
the entire action gap), and the shared-trunk Huber fit erased the
inter-action differences almost immediately: greedy switches 82 → 0
by update 3 (1.5k env steps, within ~500 gradient steps of the first
update), Q-gap pinned at ~0.2 for the rest of the run (vs ~5 for the
committed scratch arms). One brief recovery near update 18 (13.6/200)
dies again. And once the greedy policy is constant with epsilon ≤ 0.05,
the buffer refills with route-0-only experience, so the erased action
gaps can never be re-estimated — the off-policy analogue of Phase 8's
"no gradient path leads back": here it is the data distribution, not
the gradient, that seals the collapse. The landing point is telling,
too: dqn-bc TCP is byte-identical to best-static route 0 (0/10/0 tie),
where Phase 8's bc-ppo ended *below* it (−27.9, 1/9) — even this
arm's failure is value-accurate.

What the completed 2×2 pins down: the binding constraint is not
on-policy vs off-policy per se but the **return-noise-to-action-gap
ratio**. Off-policy averaging fixes everything value-scale-sized
(route selection everywhere; switching retention in the UDP cell,
where the TD noise is small enough for an O(1) action gap to persist),
and still loses the switching policy exactly where the per-step value
noise exceeds the margin between actions (TCP). Exploration is also
confirmed as a second, independent wall: no optimizer in the series
has *found* switching from scratch, because neither entropy-regularised
sampling nor epsilon-greedy dithering ever emits a coherent switching
episode to learn from. And the scripted teacher remains unbeaten in
both cells by every learned policy, four phases in.

Reproduction: `python run_benchmark.py --study offpolicy` retrains
both arms per cell (~13 min/cell) and evaluates them on the shared
seeds (the dqn-bc arm requires the committed Phase 8 `bc_<cell>.pt`
checkpoints); the four trained checkpoints are committed in
`results/checkpoints/` (~110 KB each). Then
...
The Phase 9 study: `python run_benchmark.py --study offpolicy` (then
`parse_traces.py --study offpolicy [--paired]` and `plot_results.py
--study offpolicy`). The parser and plots pull the Phase 7/8 baselines
