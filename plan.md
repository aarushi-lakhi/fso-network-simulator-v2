...
| 10 — Route-aware observation | `feat/held-route-obs` | ✅ Complete | Hypothesis confirmed (representation half): BC accuracy 0.63→0.92+, bc-route matches the teacher and **beats best-static significantly under UDP** (+99.9, 7/0/3, p≈0.03) — the first learned policy to do either. dqn-bc-route turns Phase 9's tie into a borderline win (+74.3, 8/2). Still true: TCP noise defeats every optimizer, and nothing finds switching from scratch. Analysis in `benchmarks/results/README.md` |
